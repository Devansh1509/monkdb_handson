...
#!/usr/bin/env python3
"""
silver_transform.py — MonkDB Silver Layer ETL + CSV Export + Debug Logs
-----------------------------------------------------------------------
Transforms raw user stream events into star schema (dim + fact),
updates watermark, and exports all tables to CSV snapshots.
"""
...
EXPORT_DIR = "/home/devansh/monkdb_handson/data_exports"

# -------------------------------
# HELPERS
# -------------------------------
def log(msg: str):
    """Always flush console output immediately."""
...
def normalize_ts(ts):
    """Safely convert timestamps to UTC strings."""
    if ts is None:
        return datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")
...
def load_new_raw(cur, since_ts):
    ...
    rows = cur.fetchall()
    log(f"📥 Loaded {len(rows)} new raw events since {since_ts}")
    return rows

# -------------------------------
# DIM TABLE UPSERTS
# -------------------------------
def upsert_dim(conn, cur, name, key_field, insert_sql, generator):
    data = generator()
    for key, values in data.items():
        cur.execute(f"DELETE FROM {name} WHERE {key_field}=?", (key,))
        cur.execute(insert_sql, values)
    conn.commit()
    log(f"✅ {name.split('.')[-1]}: {len(data)} upserted")

def extract_dims(events):
    dims = {
        "company": {},
        "branch": {},
        "manager": {},
    ...
    for e in events:
        dims["company"][e[14]] = (e[14], e[15], "General")
        dims["branch"][e[16]] = (e[16], e[17], e[14], e[11])
        dims["manager"][e[19]] = (e[19], e[20], e[16], e[18])
        dims["employee"][e[21]] = (e[21], e[22], e[19], e[16], e[23])
        dims["project"][e[23]] = (e[23], e[24], e[14], e[18])
    return dims
...
def load_fact(conn, cur, events):
    ...
    for e in events:
        try:
            ...
            """, (
                event_id, user_id, employee_id, project_code, float(watch_time or 0.0),
                session_id, normalize_ts(event_ts), normalize_ts(ingestion_ts),
                company_id, branch_id, manager_id, region_code
            ))
            inserted += 1
        except Exception as ex:
    ...
    conn.commit()
    log(f"✅ fact_user_activity: {inserted} inserted")

# -------------------------------
# EXPORT TO CSV
# -------------------------------
def export_to_csv(conn, tables):
    ...
    for tbl in tables:
        ...
        rows = cur.fetchall()
        if not rows:
            log(f"⚠️ Skipped {tbl} (no rows)")
            continue

        headers = [desc[0] for desc in cur.description]
        out_file = os.path.join(EXPORT_DIR, f"{tbl.split('.')[-1]}.csv")
        with open(out_file, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(headers)
            writer.writerows(rows)

        log(f"📁 Exported {tbl} → {out_file} ({len(rows)} rows)")

# -------------------------------
# MAIN
# -------------------------------
def main():
    log("🚀 Starting Silver Transform...")
    conn = connect()
    cur = conn.cursor()
    ...
    if not new_events:
        log("✅ No new data found. Exiting gracefully.")
        return

    dims = extract_dims(new_events)
    upsert_dim(conn, cur, DIM_COMPANY, "company_id",
               f"INSERT INTO {DIM_COMPANY} (company_id, company_name, industry) VALUES (?, ?, ?)",
               lambda: dims["company"])
    upsert_dim(conn, cur, DIM_BRANCH, "branch_id",
               f"INSERT INTO {DIM_BRANCH} (branch_id, branch_name, company_id, region_code) VALUES (?, ?, ?, ?)",
               lambda: dims["branch"])
    upsert_dim(conn, cur, DIM_MANAGER, "manager_id",
               f"INSERT INTO {DIM_MANAGER} (manager_id, manager_name, branch_id, department) VALUES (?, ?, ?, ?)",
               lambda: dims["manager"])
    upsert_dim(conn, cur, DIM_EMPLOYEE, "employee_id",
               f"INSERT INTO {DIM_EMPLOYEE} (employee_id, employee_name, manager_id, branch_id, project_code) VALUES (?, ?, ?, ?, ?)",
               lambda: dims["employee"])
    upsert_dim(conn, cur, DIM_PROJECT, "project_code",
               f"INSERT INTO {DIM_PROJECT} (project_code, project_name, company_id, department) VALUES (?, ?, ?, ?)",
               lambda: dims["project"])

    load_fact(conn, cur, new_events)

    last_ts = new_events[-1][1]
    update_watermark(conn, cur, last_ts)

    export_to_csv(conn, [DIM_COMPANY, DIM_BRANCH, DIM_MANAGER, DIM_EMPLOYEE, DIM_PROJECT, FACT])

    log("\n🎯 Silver Transform Complete — All data synced and exported.\n")
